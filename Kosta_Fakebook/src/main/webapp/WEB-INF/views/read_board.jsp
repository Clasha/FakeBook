				...
				<c:if test="${start >= 0 }">
					...
					<c:forEach begin="${start }" end="${end+1 }">
						...
						<c:if test="${start > 0 }"> 
							...
							<c:if test="${end <= 0 }"> 
								<c:set var="length" value="${fn:length(tag) }" /> 
								<c:set var="tag2" value="${fn:substring(tag, 1, length) }" />  
							</c:if>
							<c:if test="${end > 0 }">
								<c:set var="tag2" value="${fn:substring(tag, 1, end) }" /> <%-- tag2: ${tag2 } <br> --%>
							</c:if>							
			...
			<tr>
				<td colspan="2">
						...
						<div class="reply_list_container${board.board_num }"
							style="overflow: scroll; height: 150px; display: none; background-color: #E1E3E4;">
							<c:forEach var="reply" items="${board.replyList}" varStatus="i">
								<div class="reply_item"
									style="border: 1px solid #F1F1F1; margin: 1px; margin-top: 5px; height: 40px;">
									...
									<div>
										<a href="profile?num=${reply.m_num }">${reply.r_name}</a> ${reply.r_date }</div>
									<script type="text/javascript">
